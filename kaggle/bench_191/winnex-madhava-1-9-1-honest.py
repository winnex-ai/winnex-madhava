#!/usr/bin/env python3
"""
winnex-madhava 1.9.1 benchmark — single file. Installs the package from PyPI
...
HONEST PROTOCOL (inherited from 1-8-8-honest):
  - `pip install winnex-madhava==1.9.1` from PyPI (nothing else).
...
NEW IN 1.9.1 (the witness certificate audit):
  - For each query we also call `search_audited` (new in 1.9.0, fixed in 1.9.1) and validate
...
print("=" * 70, flush=True)
print("winnex-madhava 1.9.1 — PyPI install + per-doc certificate validation")
print("=" * 70, flush=True)

# ---- 1. Install from PyPI ----
print("\n[1] Installing winnex-madhava==1.9.1 from PyPI...", flush=True)
subprocess.check_call([sys.executable, "-m", "pip", "install", "-q",
                       "--no-cache-dir", "winnex-madhava==1.9.1"])
import winnex_madhava as wm
print(f"    winnex-madhava {wm.__version__} installed from PyPI", flush=True)
assert wm.__version__ == "1.9.1", "expected 1.9.1, got " + wm.__version__
...
def validate_certificate(eng, Qn, K=10, max_audit_records=500):
    ...
    for j in range(n_queries):
        ...
        # 3. Consistency (1.9.1 witness): with full coverage, the certificate
        #    IS the motor's decision — audit_excluded == pruned_by_bound.
        n = eng.num_vectors()
